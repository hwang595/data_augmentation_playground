...
def prepare_train_data(padding_size):
    ...
    sampled_train_images, sampled_train_labels = down_sample(data, label, down_sample_num=1000)
...
#    train_labels_new = train_labels_new[order]
    return train_set_new, train_labels_new, sampled_train_images, sampled_train_labels
...
def fill_feed_dict(all_data, all_labels, image_placeholder, label_placeholder, 
    ...
    train_batch_data, train_batch_labels, local_data_batch_idx, epoch_counter, new_train_data, new_train_labels = generate_augment_train_batch(
    ...
    return new_train_data, new_train_labels, epoch_counter, local_data_batch_idx, feed_dict
...
def generate_augment_train_batch(train_data, train_labels, train_batch_size, local_data_batch_idx, epoch_counter,
    ...
    if local_data_batch_idx > num_of_instances:
      # Finished epoch
      epoch_counter += 1
      # At start of each epoch, generate the augmented training set again
      train_data, train_labels = aug_data_set(sampled_train_images, sampled_train_labels, times_expand=1, aug_type='line_among_labels')
      # Shuffle the data
...
#    tf.logging.info("Standardized batch shapes %s" % str(whitening_image(train_batch).shape))
    ...
    return train_batch, batch_labels, local_data_batch_idx, epoch_counter, train_data, train_labels