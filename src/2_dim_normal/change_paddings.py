...
def aug_data_set(ori_data, ori_labels, times_expand=1, aug_type="crop", padding_list=None):
	new_data_table = []
	new_labels_table = []
	for pl in padding_list:
		new_data=ori_data
		new_label=ori_labels
		aug_data_list = []
		for time_aug in range(times_expand):
			if aug_type == "crop": 
				crop_data = random_crop(ori_data, crop_shape=(8, 8), padding=pl)
		...
		new_data_table.append(new_data)
		new_labels_table.append(new_label)
	return new_data_table, new_labels_table
'''
def make_it_squre_num():
	threshold = 46
	for i in range(threshold):
		indices=np.random.randint(low=0, high=IMG_HEIGHT*IMG_WIDTH, size=3)
		print(indices)
'''
...
if __name__ == "__main__":
	...
	test_set_binary_exp=expand_features(test_set_binary)
#	sample_interval = [i for i in range(5, 280, 10)]
	aug_interval = [j for j in range(1, 50, 2)]
	sample_data, sample_labels = down_sample(train_set_binary, train_labels_binary, down_sample_num=20)
	...
	acc_list = []
	sample_data_exp = expand_features(sample_data)
	raw_model = LogisticRegression(max_iter=500)
	raw_model.fit(sample_data_exp, sample_labels)
	raw_accuracy = raw_model.score(test_set_binary_exp, test_labels_binary)
	print(raw_accuracy)
	for j in aug_interval:
#		sample_data, sample_labels = down_sample(train_set_binary, train_labels_binary, down_sample_num=i)	
		new_data_table, new_labels_table = aug_data_set(sample_data, sample_labels, times_expand=j, padding_list=padding_list)
		tmp_acc = []
		for t in range(len(padding_list)):
			new_data = new_data_table[t]
			new_labels = new_labels_table[t]
			new_data_exp = expand_features(new_data)
			print(new_data_exp.shape)
			new_model = LogisticRegression(max_iter=500)
			new_model.fit(new_data_exp, new_labels)
			new_accuracy = new_model.score(test_set_binary_exp, test_labels_binary)
			tmp_acc.append(new_accuracy)
		print('=====================================================================')
		acc_list.append(tmp_acc)
#	print(acc_list)
	acc_0 = [a[0] for a in acc_list]
	acc_1 = [a[1] for a in acc_list]
	acc_2 = [a[2] for a in acc_list]
	acc_3 = [a[3] for a in acc_list]
	acc_4 = [a[4] for a in acc_list]
	acc_5 = [a[5] for a in acc_list]
	print(acc_0)
	print(acc_1)
	print(acc_2)
	print(acc_3)
