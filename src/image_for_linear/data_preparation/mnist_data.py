...
#    numpy.random.seed(int(time.time()))
  ...
  @property
  def get_flattened_feature_num(self):
    return self._reshape_feature
  '''just for this example, may not make sense in image classification'''
  @property
  def data_point_shape(self):
    return self._images[0].shape
  @property
  def test_print(self):
  #  print(numpy.amin(self._images[0]), numpy.amax(self._images[0]))
...
def load_mnist(train_dir='MNIST-data', worker_id=-1, n_workers=-1, reshape=False):
  return read_data_sets(train_dir, worker_id=worker_id, n_workers=n_workers, reshape=reshape)
