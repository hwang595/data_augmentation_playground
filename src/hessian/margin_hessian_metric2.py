...
#	x = np.array([[-1, 1, 1]])
	...
	pos_data_points, _=rand_point_generator(point_num=50)
	x = pos_data_points
...
#	interval_0 = [0, math.pi/10, math.pi/9, math.pi/8, math.pi/7, math.pi/6, math.pi/5, math.pi/4, math.pi/3, 2*math.pi/5,3*math.pi/7, math.pi/2]
	...
	for angle in interval_0:
		...
		largest_eig_sigmoid.append(np.amax(np.linalg.eig(H_max_0)[0])+np.amax(np.linalg.eig(H_max_1)[0]))
...
#		H_max_1 = nd.Hessian(log_loss_1)(np.transpose(w))
		largest_eig_crossentropy.append(np.amax(np.linalg.eig(H_max_0)[0])+np.amax(np.linalg.eig(H_max_1)[0]))
	for w in w_list_1:
		margin = min([abs(np.dot(np.transpose(w), (2,0))), abs(np.dot(np.transpose(w), (0, 2)))])
	...
	for angle in interval_2:
		...
		largest_eig_logcosh.append(np.amax(np.linalg.eig(H_max_0)[0])+np.amax(np.linalg.eig(H_max_1)[0]))
#		H = nd.Hessian(logcosh)([float(w[0]), float(w[1])])
