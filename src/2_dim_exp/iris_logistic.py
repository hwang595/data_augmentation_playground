...
#	X_train, X_test, y_train, y_test=train_test_split(train_data, train_label, test_size=0.2, random_state=42)
	# do training without regularization
	logistic_classifier = LogisticRegression(C=_C, penalty='l2')
	logistic_classifier.fit(train_data, train_label)
	plot_decision_boundary(clf=logistic_classifier,
							train_data=train_data,
							train_label=train_label)

	# let's add some regularization here, say l1
	logistic_classifier_l1 = LogisticRegression(C=1, penalty='l1')
	logistic_classifier_l1.fit(train_data, train_label)
#	svc_pred=perceptron_classifier.predict(X_test)
#	gen_err=accuracy_score(y_test, svc_pred)

	plot_decision_boundary(clf=logistic_classifier_l1,
							train_data=train_data,
							train_label=train_label)
	# l2 regularization
	logistic_classifier_l2 = LogisticRegression(C=1, penalty='l2')
	logistic_classifier_l2.fit(train_data, train_label)
	plot_decision_boundary(clf=logistic_classifier_l2,
							train_data=train_data,
							train_label=train_label)
...
#	X_train_aug, X_test_aug, y_train_aug, y_test_aug=train_test_split(aug_train_data, aug_train_label, test_size=0.2, random_state=42)
	logistic_classifier_aug = LogisticRegression()
	logistic_classifier_aug.fit(aug_train_data, aug_train_label)
...
#	print gen_err, gen_aug_err
	plot_decision_boundary(clf=logistic_classifier_aug,
	...
	logistic_classifier_fp = LogisticRegression()
	logistic_classifier_fp.fit(rand_data_fp, rand_label_fp)
	plot_decision_boundary(clf=logistic_classifier_fp,
